...
class DocumentoAdministrativoFilterSet(django_filters.FilterSet):
    ...
    tramitacao = django_filters.ChoiceFilter(
        required=False, label="Em Tramitação?", choices=YES_NO_CHOICES
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        row2 = to_row([("numero", 5), ("complemento", 2), ("ano", 5)])

        row3 = to_row([("protocolo__numero", 4), ("numero_externo", 4), ("data", 4)])

        row4 = to_row([("interessado", 6), ("assunto", 6)])
...
class ProtocoloDocumentoForm(ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper.layout = Layout(
            Fieldset(_("Identificação de Documento"), row1, row2),
            fieldset,
            row4,
            row5,
            HTML("&nbsp;"),
            Fieldset(HTML("&nbsp;"), form_actions(label=_("Protocolar Documento"))),
        )
        super(ProtocoloDocumentoForm, self).__init__(*args, **kwargs)
...
class ProtocoloMateriaForm(ModelForm):
        ...
        if self.is_valid():
            if data["vincular_materia"] == "True":
                try:
                    ...
                    self.logger.debug(
                        "Tentando obter MateriaLegislativa com ano={}, numero={} e data={}.".format(
                            data["ano_materia"],
                            data["numero_materia"],
                            data["tipo_materia"],
                        )
                    )
                    self.materia = MateriaLegislativa.objects.get(
                    ...
                    if self.materia.numero_protocolo:
                        self.logger.error(
                            "MateriaLegislativa informada já possui o protocolo {}/{} vinculado.".format(
                                self.materia.numero_protocolo, self.materia.ano
                            )
                        )
                        raise ValidationError(
                            _(
                                "Matéria Legislativa informada já possui o protocolo {}/{} vinculado.".format(
                                    self.materia.numero_protocolo, self.materia.ano
                                )
                            )
                        )
                    ...
                    self.logger.error(
                        "MateriaLegislativa informada (ano={}, numero={} e data={}) não existente.".format(
                            data["ano_materia"],
                            data["numero_materia"],
                            data["tipo_materia"],
                        )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper.layout = Layout(
            ...
            row5,
            HTML("&nbsp;"),
            Fieldset(HTML("&nbsp;"), form_actions(label=_("Protocolar Matéria"))),
        )
...
class AnexadoEmLoteFilterSet(django_filters.FilterSet):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.form.helper.layout = Layout(
            Fieldset(
                _("Pesquisa de Documentos"), row1, row2, form_actions(label="Pesquisar")
            )
        )
...
class DocumentoAdministrativoForm(FileFieldCheckMixin, ModelForm):
        ...
        if not self.instance.pk or mudanca_doc:
            ...
            if doc_exists:
                self.logger.error(
                    ...
                    "(numero={}, tipo={}, ano={}, "
                    "complemento={}) já existe.".format(
                        numero_documento, tipo_documento, ano_documento, complemento
                    )
                )
                tipo = TipoDocumentoAdministrativo.objects.get(id=tipo_documento)
                raise ValidationError(
                    _(
                        "{}/{} ({}) já existente! "
                        "Você diferenciar preenchendo o campo complemento".format(
                            numero_documento, ano_documento, tipo
                        )
                    )
                )
    ...
    def __init__(self, *args, **kwargs):
        row1 = to_row([("tipo", 3), ("numero", 3), ("complemento", 3), ("ano", 3)])
