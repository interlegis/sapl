...
class Protocolo(models.Model):
    ...
    ip_data_hora_manual = models.CharField(
        ...
        verbose_name=_("IP"),
        help_text=_(
            "Endereço IP da estação de trabalho do usuário que está realizando Protocolo e "
...
class DocumentoAdministrativo(models.Model):
    ...
    complemento = models.CharField(
        max_length=10, blank=True, verbose_name=_("Complemento")
    )
    ...
    restrito = models.BooleanField(
        default=False, verbose_name=_("Acesso Restrito"), blank=True
    )
            ...
            if k in values.keys():
                if i > 0 and campos_escolhidos[i - 1] in condicionais:
                    mask = mask.replace(
                        ...
                        condicionais[campos_escolhidos[i - 1]] if values[k] else "",
                        1,
                    )
                mask = mask.replace(k, values[k], 1)
            elif k in condicionais:
                if i > 0 and campos_escolhidos[i - 1] in condicionais:
                    mask = mask.replace(campos_escolhidos[i - 1], "", 1)
...
class VinculoDocAdminMateria(models.Model):
    documento = models.ForeignKey(
        DocumentoAdministrativo,
        related_name="materialegislativa_vinculada_set",
        on_delete=models.CASCADE,
        verbose_name=_("Documento Administrativo"),
    )
    materia = models.ForeignKey(
        MateriaLegislativa,
        related_name="documentoadministrativo_vinculado_set",
        on_delete=models.CASCADE,
