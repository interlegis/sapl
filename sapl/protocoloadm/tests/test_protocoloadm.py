...
def test_create_tramitacao(admin_client):
    tipo_doc = baker.make(TipoDocumentoAdministrativo, descricao="Teste Tipo_DocAdm")

    documento_adm = baker.make(DocumentoAdministrativo, tipo=tipo_doc)

    unidade_tramitacao_local_1 = baker.make(UnidadeTramitacao, pk=1)

    unidade_tramitacao_destino_1 = baker.make(UnidadeTramitacao, pk=2)

    unidade_tramitacao_destino_2 = baker.make(UnidadeTramitacao, pk=3)

    status = baker.make(StatusTramitacaoAdministrativo)
...
def test_anular_protocolo_form_anula_protocolo_com_doc_vinculado():
    ...
    baker.make(Protocolo, numero=1, ano=2017, tipo_materia=tipo_materia, anulado=False)

    baker.make(MateriaLegislativa, ano=2017, numero_protocolo=1)
    ...
    protocolo_documento = baker.make(
        Protocolo, numero=2, ano=2017, tipo_documento=tipo_documento, anulado=False
    )

    baker.make(DocumentoAdministrativo, protocolo=protocolo_documento)
...
def test_lista_documentos_anexados():
    tipo_documento = baker.make(TipoDocumentoAdministrativo, descricao="Tipo_Teste")
...
def test_tramitacoes_documentos_anexados(admin_client):
    ...
    status = baker.make(StatusTramitacaoAdministrativo, indicador="R")
