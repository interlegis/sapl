...
from sapl.protocoloadm.forms import (VinculoDocAdminMateriaEmLoteFilterSet,
...
from .forms import (AcompanhamentoDocumentoForm, AnexadoEmLoteFilterSet,
                    AnexadoForm, AnularProtocoloAdmForm,
                    DesvincularDocumentoForm, DesvincularMateriaForm,
                    DocumentoAcessorioAdministrativoForm,
                    DocumentoAdministrativoFilterSet,
                    DocumentoAdministrativoForm, FichaPesquisaAdmForm,
                    FichaSelecionaAdmForm,
                    PrimeiraTramitacaoEmLoteAdmFilterSet,
                    ProtocoloDocumentoForm, ProtocoloFilterSet,
                    ProtocoloMateriaForm, TramitacaoAdmEditForm,
                    TramitacaoAdmForm, TramitacaoEmLoteAdmFilterSet,
                    TramitacaoEmLoteAdmForm, compara_tramitacoes_doc,
...
def create_pdf_docacessorios(docadministrativo):
    ...
    """
    logger = logging.getLogger(__name__)
    docs = docadministrativo.documentoacessorioadministrativo_set.all().values_list(
        "arquivo", flat=True
    )
    if not docs:
        return None, None
...
class DocumentoAdministrativoCrud(Crud):
    ...
    class UpdateView(Crud.UpdateView):
        ...
        def get_initial(self):
            if self.object.protocolo:
                ...
                return {"ano_protocolo": p.ano, "numero_protocolo": p.numero}
    ...
    class DetailView(DocumentoAdministrativoMixin, Crud.DetailView):
        ...
        def urlize(self, obj, fieldname):
            a = '<a href="%s">%s</a>' % (
                reverse("sapl.protocoloadm:doc_texto_integral", kwargs={"pk": obj.pk}),
...
class ProtocoloDocumentoView(
    ...
    def form_valid(self, form):
        ...
        if not numeracao:
            self.logger.error(
                "user=" + username + ". É preciso definir a sequencia de "
                "numeração na tabelas auxiliares! "
            )
            msg = _(
                "É preciso definir a sequencia de " + "numeração na tabelas auxiliares!"
            )
...
class PesquisarDocumentoAdministrativoView(
    ...
    def get_filterset_kwargs(self, filterset_class):
        super(PesquisarDocumentoAdministrativoView, self).get_filterset_kwargs(
            filterset_class
        )

        kwargs = {"data": self.request.GET or None}

        status_tramitacao = self.request.GET.get("tramitacaoadministrativo__status")
    ...
    def get_context_data(self, **kwargs):
        context = super(PesquisarDocumentoAdministrativoView, self).get_context_data(
            **kwargs
        )
...
class TramitacaoAdmCrud(MasterDetailCrud):
    ...
    class DetailView(DocumentoAdministrativoMixin, MasterDetailCrud.DetailView):
    ...
    class DeleteView(MasterDetailCrud.DeleteView):
        ...
        def delete(self, request, *args, **kwargs):
            ...
            if tramitacao.pk != ultima_tramitacao.pk:
                username = request.user.username
                self.logger.warning(
                    "User={}. Não é possível deletar a tramitação de pk={}. "
                    "Somente a última tramitação (pk={}) pode ser deletada!.".format(
                        username, tramitacao.pk, ultima_tramitacao.pk
                    )
                )
                    ...
                    for da in docs_anexados:
                        ...
                        if compara_tramitacoes_doc(tram_anexada, tramitacao):
                            ...
                            if da.tramitacaoadministrativo_set.count() == 0:
                                ...
                                da.save()
                TramitacaoAdministrativo.objects.filter(
                    id__in=[t.id for t in tramitacoes_deletar]
                ).delete()
...
def apaga_protocolos(request, ano, numero_protocolo=None):
    ...
    for ml in MateriaLegislativa.objects.filter(
        ano__in=ano, numero_protocolo__in=all_protocolos.values_list("numero")
    ):
...
def apaga_protocolos_view(request):
    if request.method == "GET":
        ...
        return render(
            request,
            "protocoloadm/deleta_todos_protocolos.html",
