...
from sapl.sessao.models import RegistroVotacao, SessaoPlenaria, VotoParlamentar
...
class RelatorioDocumentosAcessoriosFilterSet(django_filters.FilterSet):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.form.helper.layout = Layout(Fieldset(_("Pesquisa"), row0, row1, buttons))
...
class RelatorioNormasMesFilterSet(django_filters.FilterSet):
    ...
    tipo = django_filters.ChoiceFilter(
        required=False, label="Tipo Norma", choices=choice_tipos_normas, initial=0
    )
...
class RelatorioDataFimPrazoTramitacaoFilterSet(django_filters.FilterSet):
    ...
    def qs(self):
        ...
        return (
            parent.distinct()
            .prefetch_related("materia__tipo")
            .order_by("tramitacao__data_fim_prazo", "materia__tipo", "materia__numero")
        )
    ...
    def __init__(self, *args, **kwargs):
        ...
        row4 = to_row([("materia__tipo", 6), ("tramitacao__status", 6)])
...
class RelatorioMateriasTramitacaoFilterSet(django_filters.FilterSet):
    ...
    materia__autores = django_filters.ModelChoiceFilter(
        label="Autor da Matéria", queryset=Autor.objects.all()
    )
    ...
    def __init__(self, *args, **kwargs):
        super(RelatorioMateriasTramitacaoFilterSet, self).__init__(*args, **kwargs)
...
class RelatorioMateriasPorAnoAutorTipoFilterSet(django_filters.FilterSet):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.form.helper.layout = Layout(
            Fieldset(
                _("Pesquisa de Matéria por Ano Autor Tipo"),
                row1,
                buttons,
            )
        )
...
class RelatorioMateriasPorAutorFilterSet(django_filters.FilterSet):
    ...
    def qs(self):
        parent = super().qs
        return parent.distinct().order_by(
            "-ano", "-numero", "tipo", "autoria__autor", "-autoria__primeiro_autor"
        )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.form.helper.layout = Layout(
            Fieldset(
                _("Pesquisa de Matéria por Autor"),
                row1,
                row2,
                HTML(autor_label),
                HTML(autor_modal),
