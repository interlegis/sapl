...
from sapl.base.models import AppConfig as SaplAppConfig
from sapl.base.models import Autor, CasaLegislativa
from sapl.comissoes.models import Comissao, Reuniao
from sapl.crud.base import make_pagination
from sapl.materia.models import (Autoria, ConfigEtiquetaMateriaLegislativa,
                                 DocumentoAcessorio, MateriaAssunto,
                                 MateriaEmTramitacao, MateriaLegislativa,
                                 Numeracao, StatusTramitacao, TipoDocumento,
...
from sapl.relatorios.forms import (RelatorioAtasFilterSet,
...
from sapl.sessao.models import (ConsideracoesFinais, ExpedienteMateria,
                                ExpedienteSessao, IntegranteMesa,
                                JustificativaAusencia, OcorrenciaSessao,
                                Orador, OradorExpediente, OradorOrdemDia,
                                OrdemDia, PresencaOrdemDia, RegistroVotacao,
                                ResumoOrdenacao, SessaoPlenaria,
                                SessaoPlenariaPresenca, TipoSessaoPlenaria,
                                VotoParlamentar)
from sapl.sessao.views import (get_assinaturas, get_consideracoes_finais,
                               get_correspondencias, get_expedientes,
...
from sapl.utils import (LISTA_DE_UFS, TrocaTag, create_barcode, filiacao_data,
...
def get_sessao_plenaria(sessao, casa, user):
    inf_basicas_dic = {
        ...
        "dat_fim_sessao": sessao.data_fim.strftime("%d/%m/%Y")
        if sessao.data_fim
        else "",
    ...
    # Conteudo multimidia
    cont_mult_dic = {
        "multimidia_audio": str(sessao.url_audio)
        if sessao.url_audio
        else "Indisponível",
        "multimidia_video": str(sessao.url_video)
        if sessao.url_video
        else "Indisponível",
    }
    ...
    lst_mesa = []
    for composicao in (
        IntegranteMesa.objects.select_related("parlamentar", "cargo")
        .filter(sessao_plenaria=sessao)
        .order_by("cargo_id")
    ):
        partido_sigla = Filiacao.objects.filter(
            parlamentar=composicao.parlamentar
        ).first()
    ...
    for expediente_materia in ExpedienteMateria.objects.select_related(
        "materia"
    ).filter(sessao_plenaria=sessao):
        ...
        dic_expediente_materia.update(
            {"nom_resultado": resultado, "votacao_observacao": resultado_observacao}
        )
    ...
    materias_expediente_votacao_nominal = ExpedienteMateria.objects.filter(
        sessao_plenaria=sessao, tipo_votacao=2
    ).order_by("-materia")
    ...
    for mevn in materias_expediente_votacao_nominal:
        ...
        lst_expediente_materia_vot_nom.append(
            {"titulo": titulo_materia, "votos": votos_materia}
        )
    ...
    for votacao in OrdemDia.objects.filter(sessao_plenaria=sessao):
        ...
        rv = votacao.registrovotacao_set.filter(materia=votacao.materia).first()
    ...
    for modvn in materias_ordem_dia_votacao_nominal:
        ...
        lst_votacao_vot_nom.append({"titulo": t_materia, "votos": votos_materia_od})
    ...
    lst_oradores = []
    for orador in (
        Orador.objects.select_related("parlamentar")
        .filter(sessao_plenaria=sessao)
        .order_by("numero_ordem")
    ):
...
def relatorio_sessao_plenaria(request, pk):
    ...
    (
        ...
        lst_ocorrencias,
        lst_consideracoes,
    ) = get_sessao_plenaria(sessao, casa, request.user)
...
def get_pauta_sessao(sessao, casa):
    ...
    for votacao in OrdemDia.objects.filter(sessao_plenaria=sessao):
        ...
        dic_votacao["id_materia"] = str(materia.numero) + "/" + str(materia.ano)
...
def relatorio_doc_administrativos(request, context):
    return cria_relatorio(
        request, context, "relatorios/relatorio_doc_administrativos.html"
    )


def relatorio_materia_em_tramitacao(obj, request, context):
    return cria_relatorio(
        request, context, "relatorios/relatorio_materias_em_tramitacao.html"
    )


def relatorio_materia_por_autor(obj, request, context):
    return cria_relatorio(
        request, context, "relatorios/relatorio_materias_por_autor.html"
    )


def relatorio_materia_por_ano_autor(obj, request, context):
    return cria_relatorio(
        request, context, "relatorios/relatorio_materias_por_ano_autor.html"
    )
...
def relatorio_historico_tramitacao(obj, request, context):
    return cria_relatorio(
        request, context, "relatorios/relatorio_historico_tramitacao.html"
    )


def relatorio_fim_prazo_tramitacao(obj, request, context):
    return cria_relatorio(
        request, context, "relatorios/relatorio_fim_prazo_tramitacao.html"
    )
...
def relatorio_historico_tramitacao_adm(obj, request, context):
    return cria_relatorio(
        request, context, "relatorios/relatorio_historico_tramitacao_adm.html"
    )


def relatorio_estatisticas_acesso_normas(obj, request, context):
    return cria_relatorio(
        request, context, "relatorios/relatorio_estatisticas_acesso_normas.html"
    )


def relatorio_documento_acessorio(obj, request, context):
    return cria_relatorio(
        request, context, "relatorios/relatorio_documento_acessorio.html"
    )


def relatorio_normas_por_autor(obj, request, context):
    return cria_relatorio(
        request, context, "relatorios/relatorio_normas_por_autor.html"
    )
...
def relatorio_pauta_sessao_weasy(obj, request, context):
    ...
    return cria_relatorio(
        request, context, "relatorios/relatorio_pauta_sessao.html", info
    )
...
def relatorio_sessao_plenaria_pdf(request, pk):
    ...
    info = "Resumo da {}ª Reunião {} \
                ...
                Legislatura".format(
        inf_basicas_dic["num_sessao_plen"],
    ...
    html_header = render_to_string(
        "relatorios/header_ata.html",
        {"casa": casa, "MEDIA_URL": MEDIA_URL, "logotipo": casa.logotipo, "info": info},
    )
...
def gera_etiqueta_ml(materia_legislativa, base_url):
    ...
    ml_info = unidecode.unidecode(
        "{}/{}-{}".format(
            materia_legislativa.numero,
    ...
    ementa = (
        ementa if len(ementa) < max_ementa_size else ementa[:max_ementa_size] + "..."
    )
    ...
    main_doc = html.render(
        stylesheets=[
            CSS(string="@page {{size: {}cm {}cm;}}".format(confg.largura, confg.altura))
        ]
    )
...
class RelatorioHistoricoTramitacaoView(RelatorioMixin, FilterView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.GET["autoria__autor"]:
            context["autoria__autor"] = str(
                Autor.objects.get(id=self.request.GET["autoria__autor"])
            )
        else:
...
class RelatorioMateriasTramitacaoView(RelatorioMixin, FilterView):
    ...
    def get_filterset_kwargs(self, filterset_class):
        ...
        if data["data"]:
            ...
            unidade_tramitacao_destino = data["data"][
    ...
    def get_queryset(self):
        ...
        qs = (
            qs.select_related("materia__tipo")
            .filter(materia__em_tramitacao=True)
            .exclude(tramitacao__status__indicador="F")
...
class RelatorioMateriaAnoAssuntoView(ListView):
    ...
    def get_queryset(self):
        return (
            MateriaAssunto.objects.all()
            .values(
                "assunto_id",
                assunto_materia=F("assunto__assunto"),
                ano=F("materia__ano"),
            )
            .annotate(total=Count("assunto_id"))
            .order_by("-materia__ano", "assunto_id")
        )
    ...
    def get_context_data(self, **kwargs):
        ...
        mat = (
            MateriaLegislativa.objects.filter(materiaassunto__isnull=True)
            .values("ano")
    ...
    def get_filterset_kwargs(self, filterset_class):
        ...
        if kwargs["data"]:
            ...
            if vigencia == "True":
                ...
                qs = (
                    qs_dt_not_null | qs.filter(data_vigencia__gte=datetime.now().date())
                ).distinct()
            else:
                qs = qs.filter(data_vigencia__lt=datetime.now().date())
