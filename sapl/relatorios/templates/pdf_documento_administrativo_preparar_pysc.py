...
documentos = []
REQUEST = context.REQUEST
for documento in context.zsql.documento_administrativo_pesquisar_zsql(
    tip_documento=REQUEST["lst_tip_documento"],
    num_documento=REQUEST["txt_num_documento"],
    ano_documento=REQUEST["txt_ano_documento"],
    num_protocolo=REQUEST["txt_num_protocolo"],
    ind_tramitacao=REQUEST["rad_tramitando"],
    des_assunto=REQUEST["txa_txt_assunto"],
    cod_status=REQUEST["lst_status"],
    txt_interessado=REQUEST["txa_txt_interessado"],
    dat_apres1=REQUEST["dt_apres1"],
    ...
    dic["titulo"] = (
        documento.sgl_tipo_documento
        + " "
        + str(documento.num_documento)
        + " "
        + str(documento.ano_documento)
        + " - "
        + documento.des_tipo_documento
    )
    ...
    for tramitacao in context.zsql.tramitacao_administrativo_obter_zsql(
        cod_documento=documento.cod_documento, ind_ult_tramitacao=1
    ):
...
if REQUEST.lst_tip_documento != "":
    for tipo_documento in context.zsql.tipo_documento_administrativo_obter_zsql(
        ind_excluido=0, tip_documento=REQUEST.lst_tip_documento
    ):
        filtro["tipo_documento"] = (
            tipo_documento.sgl_tipo_documento
            + " - "
...
if REQUEST.lst_status != "":
    for status in context.zsql.status_tramitacao_administrativo_obter_zsql(
        ind_exluido=0, cod_status=REQUEST.lst_status
    ):
