...
if context.REQUEST["cod_sessao_plen"] != "":
    ...
    data = ""
    for dat_sessao in context.zsql.sessao_plenaria_obter_zsql(
        ...
        dat_ordem = context.pysc.data_converter_pysc(
            dat_sessao.dat_inicio_sessao
        )  # converte data para formato yyyy/mm/dd
    # seleciona dados da sessão plenária
    for sp in context.zsql.sessao_plenaria_obter_zsql(
        dat_inicio_sessao=data, ind_excluido=0
    ):
        dicsp = (
            {}
        )  # dicionário que armazenará os dados a serem impressos de uma sessão plenária
        ts = context.zsql.tipo_sessao_plenaria_obter_zsql(tip_sessao=sp.tip_sessao)[0]
        dicsp["sessao"] = (
            str(sp.num_sessao_plen)
            + "ª Sessao "
            + ts.nom_sessao
            + " da "
            + str(sp.num_sessao_leg)
            + "ª Sessao Legislativa da "
            + str(sp.num_legislatura)
            + "ª Legislatura"
        )
        ...
        dicsp["datasessao"] = (
            "Dia " + str(dia) + " (" + str(sp.dia_sessao) + ") - Inicio as " + hora
        )
        splen.append(dicsp)
    ...
    for ordem in context.zsql.ordem_dia_obter_zsql(dat_ordem=data, ind_excluido=0):
        ...
        for relatoria in context.zsql.relatoria_obter_zsql(
            ...
            lst_relator.append(
                parlamentar.nom_parlamentar + " - " + comissao.nom_comissao
            )
    ...
    if localidade[0].nom_localidade != None and localidade[0].nom_localidade != "":
        linha1 = (
            linha1 + "   " + localidade[0].nom_localidade + " - " + localidade[0].sgl_uf
        )
