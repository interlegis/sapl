...
protocolos = []
REQUEST = context.REQUEST
for protocolo in context.zsql.protocolo_pesquisar_zsql(
    tip_protocolo=REQUEST["rad_tip_protocolo"],
    cod_protocolo=REQUEST["txt_num_protocolo"],
    ano_protocolo=REQUEST["txt_ano_protocolo"],
    tip_documento=REQUEST["lst_tip_documento"],
    tip_processo=REQUEST["rad_tip_processo"],
    tip_materia=REQUEST["lst_tip_materia"],
    des_assunto=REQUEST["txt_assunto"],
    cod_autor=REQUEST["hdn_cod_autor"],
    des_interessado=REQUEST["txa_txt_interessado"],
    dat_apres=REQUEST["dt_apres"],
    dat_apres2=REQUEST["dt_apres2"],
):
    dic = {}
    ...
    dic["data"] = (
        context.pysc.iso_to_port_pysc(protocolo.dat_protocolo)
        + " - "
    ...
    dic["num_materia"] = ""
    for materia in context.zsql.materia_obter_zsql(
        num_protocolo=protocolo.cod_protocolo, ano_ident_basica=protocolo.ano_protocolo
    ):
        dic["num_materia"] = (
    ...
    dic["num_documento"] = ""
    for documento in context.zsql.documento_administrativo_obter_zsql(
        num_protocolo=protocolo.cod_protocolo
    ):
        dic["num_documento"] = (
    ...
    dic["numeracao"] = ""
    for materia_num in context.zsql.materia_obter_zsql(
        num_protocolo=protocolo.cod_protocolo, ano_ident_basica=protocolo.ano_protocolo
    ):
        for numera in context.zsql.numeracao_obter_zsql(
            cod_materia=materia_num.cod_materia, ind_excluido=0
        ):
            dic["numeracao"] = (
                "PROCESSO N&#176; "
                + str(numera.num_materia)
                + "/"
                + str(numera.ano_materia)
            )
