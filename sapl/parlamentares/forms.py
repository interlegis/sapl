...
from .models import (Bloco, BlocoMembro, Coligacao, ComposicaoColigacao,
                     Filiacao, Frente, FrenteParlamentar, Legislatura, Mandato,
                     Parlamentar, Partido, Votante)
...
class LegislaturaForm(ModelForm):
    ...
    class Meta:
        ...
        widgets = {"observacao": forms.Textarea(attrs={"id": "texto-rico"})}
...
class ColigacaoFilterSet(django_filters.FilterSet):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.form.helper.form_method = "GET"
        self.form.helper.layout = Layout(
            Fieldset(_("Pesquisa de Coligação"), row0, form_actions(label="Pesquisar"))
        )


class PartidoFilterSet(django_filters.FilterSet):
    nome = django_filters.CharFilter(
        label=_("Nome do Partido"), method="multifield_filter"
    )
    ...
    def multifield_filter(self, queryset, name, value):
        return queryset.filter(Q(sigla__icontains=value) | Q(nome__icontains=value))
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.form.helper.form_method = "GET"
        self.form.helper.layout = Layout(
            Fieldset(_("Pesquisa de Partido"), row0, form_actions(label="Pesquisar"))
        )
...
def validar_datas(data_filiacao, data_desfiliacao, parlamentar, filiacao):
    ...
    if data_desfiliacao and data_desfiliacao < data_filiacao:
        ...
        error_msg = _(
            "A data de desfiliação não pode anterior \
                       à data de filiação"
        )
...
class FrenteParlamentarForm(ModelForm):
    ...
    def clean(self):
        ...
        if cd["cargo"].cargo_unico:
            ...
            if (
                frente_parlamentar
                and not frente_parlamentar[0].parlamentar == cd["parlamentar"]
            ):
                raise ValidationError(
...
class VotanteForm(ModelForm):
    username = forms.CharField(label=_("Usuário"), required=True, max_length=30)
...
class BlocoMembroForm(ModelForm):
    ...
    def clean(self):
        ...
        if (
            cd["cargo"].cargo_unico
            and BlocoMembro.objects.filter(
                bloco=cd["bloco"], cargo=cd["cargo"], data_saida__isnull=True
            )
            ...
            not cd["data_saida"]
            and BlocoMembro.objects.filter(
                parlamentar=cd["parlamentar"], data_saida__isnull=True
            ).exists()
        ):
