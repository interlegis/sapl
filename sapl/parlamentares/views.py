...
from sapl.base.forms import PartidoForm, SessaoLegislativaForm
...
from .forms import (BlocoForm, BlocoMembroForm, ColigacaoFilterSet,
                    FiliacaoForm, FrenteForm, FrenteParlamentarForm,
                    LegislaturaForm, MandatoForm, ParlamentarCreateForm,
                    ParlamentarFilterSet, ParlamentarForm, PartidoFilterSet,
                    VincularParlamentarForm, VotanteForm)
from .models import (Bloco, BlocoCargo, BlocoMembro, CargoMesa, Coligacao,
                     ComposicaoColigacao, ComposicaoMesa, Dependente, Filiacao,
                     Frente, FrenteCargo, FrenteParlamentar, Legislatura,
                     Mandato, MesaDiretora, NivelInstrucao, Parlamentar,
                     Partido, SessaoLegislativa, SituacaoMilitar,
...
class PesquisarParlamentarView(FilterView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if (
            "nome_parlamentar" in self.request.META["QUERY_STRING"]
...
class PesquisarColigacaoView(FilterView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if (
            "nome" in self.request.META["QUERY_STRING"]
            or "page" in self.request.META["QUERY_STRING"]
        ):
            resultados = self.object_list
        else:
...
def get_parlamentar_frentes(request, pk):
    ...
    frentes = [
        f
        for f in FrenteParlamentar.objects.filter(parlamentar_id=pk)
...
class LegislaturaCrud(CrudAux):
    ...
    list_field_names = ["numero", "data_eleicao", "data_inicio", "data_fim"]
...
class ParlamentarCrud(Crud):
    ...
    class ListView(Crud.ListView):
        ...
        def take_legislatura_id(self):
            ...
            try:
                self.logger.debug(
                    "user=" + username + ". Tentando obter id da legislatura."
                )
                ...
                self.logger.warning(
                    "User="
                    + username
                    + ". Legislatura não possui ID. Buscando em todas as entradas."
                )
...
class ParlamentarMateriasView(FormView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            mensagem = _(
                "Este Parlamentar não está associado como autor de matéria.".format(
                    parlamentar_pk
                )
            )
            self.logger.error(
...
class ParlamentarNormasView(FormView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        autoria = (
            AutoriaNorma.objects.filter(autor=autor, primeiro_autor=True)
            .values(
                "norma__ano",
                "norma__tipo__pk",
                "norma__tipo__sigla",
            ...
            .annotate(total=Count("norma__tipo__pk"))
            .order_by("-norma__ano", "norma__tipo")
        )

        coautoria = (
            AutoriaNorma.objects.filter(autor=autor, primeiro_autor=False)
            .values(
                "norma__ano",
                "norma__tipo__pk",
                "norma__tipo__sigla",
...
class MesaDiretoraView(FormView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        mesa_diretora = (
            sessao_atual.mesadiretora_set.order_by("-data_inicio").first()
            if sessao_atual
            else None
        )

        composicao_mesa = (
            ComposicaoMesa.objects.select_related("cargo", "parlamentar")
            .filter(mesa_diretora=mesa_diretora)
...
def altera_field_mesa(request):
    ...
    # atual deve ser a primeira daquela legislatura
        ...
        year = timezone.now().year
        logger.debug(
            "user={}. Tentando obter id de sessoes com data_inicio.ano={}.".format(
                username, year
            )
        )
    ...
    if request.user.has_perm(
        ...
        try:
            ...
            parlamentar_ja_inserido = ComposicaoMesa.objects.filter(
                mesa_diretora=mesa_diretora, cargo=composicao.cargo
            ).exists()
...
def altera_field_mesa_public_view(request):
    ...
    sessoes = legislatura.sessaolegislativa_set.filter(tipo="O").order_by(
        "-data_inicio"
    )
    ...
    if not sessao_selecionada:
        ...
        if sessao_selecionada is None:
            logger.error(
                f"user={username}. Sessões não encontradas com com data_inicio.ano = {year}. "
    ...
    parlamentares_ocupados = list(
        composicao_mesa.values_list("parlamentar__id", "parlamentar__nome_parlamentar")
    )
