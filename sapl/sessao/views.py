...
from .forms import (AdicionarVariasMateriasFilterSet, BancadaForm,
                    ExpedienteForm, JustificativaAusenciaForm, ListMateriaForm,
                    MesaForm, OcorrenciaSessaoForm, OradorExpedienteForm,
                    OradorForm, OradorOrdemDiaForm, PautaSessaoFilterSet,
                    PresencaForm, ResumoOrdenacaoForm, RetiradaPautaForm,
                    SessaoPlenariaFilterSet, SessaoPlenariaForm,
                    VotacaoEditForm, VotacaoForm, VotacaoNominalForm)
from .models import (ORDENACAO_RESUMO, Bancada, CargoBancada, CargoMesa,
                     ConsideracoesFinais, ExpedienteMateria, ExpedienteSessao,
                     IntegranteMesa, JustificativaAusencia, MateriaLegislativa,
                     OcorrenciaSessao, Orador, OradorExpediente,
                     OradorOrdemDia, OrdemDia, PresencaOrdemDia,
                     RegistroLeitura, RegistroVotacao, ResumoOrdenacao,
                     RetiradaPauta, SessaoPlenaria, SessaoPlenariaPresenca,
                     TipoExpediente, TipoJustificativa, TipoResultadoVotacao,
                     TipoRetiradaPauta, TipoSessaoPlenaria, VotoParlamentar)
...
def reordena_materias(request, pk, tipo, ordenacao):
    TIPOS_MATERIAS = {"expediente": ExpedienteMateria, "ordemdia": OrdemDia}
    ...
    materias = (
        TIPOS_MATERIAS[tipo]
        .objects.filter(sessao_plenaria_id=pk)
        .order_by(*TIPOS_ORDENACAO[ordenacao])
    )
...
def verifica_votacoes_abertas(request):
    ...
    if votacoes_abertas:
        ...
        logger.info(
            ...
            + ", ".join(msg_abertas)
            + ". Estas votações ou leituras foram fechadas."
        )
        msg = _(
            "Já existem votações ou leituras abertas nas seguintes Sessões: "
            + ", ".join(msg_abertas)
...
def customize_link_materia(context, pk, has_permission, is_expediente):
    for i, row in enumerate(context["rows"]):
        ...
        numeracao = (
            materia.numeracao_set.first() if materia.numeracao_set.first() else "-"
        )
        ...
        todos_autores = (
            ", ".join([str(a.autor) for a in todos_autoria]) if autoria else "-"
        )
        ...
        data_sessao = (
            ...
            if sessao_plenaria.data_fim
            else sessao_plenaria.data_inicio
        )
        tramitacao = (
            Tramitacao.objects.select_related("materia", "status", "materia__tipo")
            .filter(
                materia=materia, turno__isnull=False, data_tramitacao__lte=data_sessao
            )
        ...
        if tramitacao:
            for t in Tramitacao.TURNO_CHOICES:
                if t[0] == tramitacao.turno:
                    turno = t[1]
                    break
        materia_em_tramitacao = (
            MateriaEmTramitacao.objects.select_related("materia", "tramitacao")
        ...
        if (
            obj.tipo_votacao != LEITURA and not exist_resultado and not exist_retirada
        ) or (obj.tipo_votacao == LEITURA and not exist_leitura):
            if obj.votacao_aberta:
                url = ""
                if is_expediente:
                    if obj.tipo_votacao == SIMBOLICA:
                        url = reverse(
                            "sapl.sessao:votacaosimbolicaexp",
                            kwargs={
            ...
            if has_permission:
                ...
                if is_expediente:
                    if obj.tipo_votacao == SIMBOLICA:
                        ...
                        url = reverse(
                            "sapl.sessao:votacaonominalexpedit",
                            kwargs={
                                "pk": obj.sessao_plenaria_id,
                                "oid": obj.pk,
                                "mid": obj.materia_id,
                        ...
                        url = reverse(
                            "sapl.sessao:votacaosecretaexpedit",
                            kwargs={
                                "pk": obj.sessao_plenaria_id,
                                "oid": obj.pk,
                                "mid": obj.materia_id,
                    ...
                    if obj.tipo_votacao == SIMBOLICA:
                        url = reverse(
                            "sapl.sessao:votacaosimbolicaedit",
                            kwargs={
                                "pk": obj.sessao_plenaria_id,
                                "oid": obj.pk,
                                "mid": obj.materia_id,
                        ...
                        url = reverse(
                            "sapl.sessao:votacaonominaledit",
                            kwargs={
                                "pk": obj.sessao_plenaria_id,
                                "oid": obj.pk,
                                "mid": obj.materia_id,
                        ...
                        url = reverse(
                            "sapl.sessao:votacaosecretaedit",
                            kwargs={
                                "pk": obj.sessao_plenaria_id,
                                "oid": obj.pk,
                                "mid": obj.materia_id,
                ...
                resultado = '<a href="%s?page=%s">%s<br/><br/>%s</a>' % (
                    url,
                    context.get("page", 1),
                    resultado_descricao,
...
class TransferenciaMateriasSessaoAbstract(PermissionRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        context = super(TransferenciaMateriasSessaoAbstract, self).get_context_data(
            **kwargs
        )
        ...
        if materias_sessao:
            context["sessoes_destino"] = (
                SessaoPlenaria.objects.filter(
                    data_inicio__gte=sessao_plenaria_atual.data_inicio
                )
                .exclude(pk=sessao_plenaria_atual.pk)
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not sessao_plenaria_destino_id:
            self.logger.error("Sessão plenária de destino inexistente.")
    ...
    class ListView(MasterDetailCrud.ListView):
        ...
        def get_context_data(self, **kwargs):
            ...
            return customize_link_materia(
                context, self.kwargs["pk"], has_permition, False
            )
...
def recuperar_tramitacao(request):
    ...
    try:
        materia = MateriaLegislativa.objects.get(tipo_id=tipo, ano=ano, numero=numero)
        tramitacao = {}
...
class ExpedienteMateriaCrud(MasterDetailCrud):
    ...
    class ListView(MasterDetailCrud.ListView):
        ...
        def get_context_data(self, **kwargs):
            ...
            has_permition = self.request.user.has_module_perms(AppConfig.label)
            return customize_link_materia(
                context, self.kwargs["pk"], has_permition, True
            )
    ...
    class CreateView(MasterDetailCrud.CreateView):
        ...
        def get_context_data(self, **kwargs):
            ...
            if tipo_sessao.nome == "Solene":
                context.update({"subnav_template_name": "sessao/subnav-solene.yaml"})
            ultimo_orador = (
                Orador.objects.filter(sessao_plenaria=kwargs["root_pk"])
    ...
    class CreateView(MasterDetailCrud.CreateView):
        ...
        def get_context_data(self, **kwargs):
            ...
            if tipo_sessao.nome == "Solene":
                context.update({"subnav_template_name": "sessao/subnav-solene.yaml"})
            ultimo_orador = (
                OradorExpediente.objects.filter(sessao_plenaria=kwargs["root_pk"])
                .order_by("-numero_ordem")
                .first()
            )
            context["ultima_ordem"] = ultimo_orador.numero_ordem if ultimo_orador else 0
...
class OradorOrdemDiaCrud(OradorCrud):
    ...
    class CreateView(MasterDetailCrud.CreateView):
        ...
        def get_context_data(self, **kwargs):
            context = super().get_context_data(**kwargs)
            ultimo_orador = (
                OradorOrdemDia.objects.filter(sessao_plenaria=kwargs["root_pk"])
                .order_by("-numero_ordem")
                .first()
            )
            context["ultima_ordem"] = ultimo_orador.numero_ordem if ultimo_orador else 0
...
def recuperar_numero_sessao_view(request):
    try:
        ...
        sessao = SessaoPlenaria.objects.filter(
            tipo.build_predicados_queryset(l, sl, data)
        ).last()
...
def get_materias_expediente(sessao_plenaria):
    materias_expediente = []
    for m in ExpedienteMateria.objects.select_related("materia").filter(
        sessao_plenaria_id=sessao_plenaria.id
    ):
        tramitacao = ""
        data_sessao = (
        ...
        for aux_tramitacao in Tramitacao.objects.filter(
            materia=m.materia, data_tramitacao__lte=data_sessao
        ).order_by("-data_tramitacao", "-id"):
        ...
        materias_expediente.append(
            {
                ...
                "situacao": materia_em_tramitacao.tramitacao.status
                if materia_em_tramitacao
                else _("Não informada"),
                "resultado": resultado,
                "resultado_observacao": resultado_observacao,
                "autor": [
                    str(x.autor)
                    for x in Autoria.objects.select_related("autor").filter(
...
def get_presenca_ordem_do_dia(sessao_plenaria):
    parlamentares_ordem = [
        p.parlamentar
        for p in PresencaOrdemDia.objects.filter(sessao_plenaria_id=sessao_plenaria.id)
        .distinct()
        .order_by("parlamentar__nome_parlamentar")
    ]
...
def get_assinaturas(sessao_plenaria):
    ...
    if config_assinatura_ata == "T" and parlamentares_ordem:
        ...
        context.update(
            {"assinatura_mesa": mesa_dia, "assinatura_presentes": parlamentares_ordem}
        )
...
def get_assinaturas_presidente(sessao_plenaria):
    ...
    context = {}
    assinatura_presidente = [{"parlamentar": presidente_dia, "cargo": "Presidente"}]
    context.update({"assinatura_mesa": assinatura_presidente})
...
def get_materias_ordem_do_dia(sessao_plenaria):
    ...
    for o in OrdemDia.objects.filter(sessao_plenaria_id=sessao_plenaria.id):
        ...
        for aux_tramitacao in Tramitacao.objects.filter(
            materia=o.materia, data_tramitacao__lte=data_sessao
        ).order_by("-data_tramitacao", "-id"):
            if aux_tramitacao.turno:
                tramitacao = aux_tramitacao
                break
        ...
        if o.tipo_votacao == 2:
            for voto in VotoParlamentar.objects.filter(ordem=o.id):
                voto_nominal.append((voto.parlamentar.nome_parlamentar, voto.voto))
...
def get_oradores_explicacoes_pessoais(sessao_plenaria):
    ...
    for orador in Orador.objects.filter(sessao_plenaria_id=sessao_plenaria.id).order_by(
        "numero_ordem"
    ):
...
class ResumoView(DetailView):
    ...
    def get_context(self, *args, **kwargs):
        ...
        votacoes = []
        for mevn in ExpedienteMateria.objects.filter(
            sessao_plenaria_id=self.object.id, tipo_votacao=2
        ).order_by("-materia"):
            votos_materia = []
            titulo_materia = mevn.materia
            registro = RegistroVotacao.objects.filter(expediente=mevn)
            if registro:
                for vp in VotoParlamentar.objects.filter(votacao__in=registro).order_by(
                    "parlamentar"
                ):
                    votos_materia.append(vp)

            votacoes.append({"titulo": titulo_materia, "votos": votos_materia})
        ...
        votacoes_od = []
        for modvn in OrdemDia.objects.filter(
            sessao_plenaria_id=self.object.id, tipo_votacao=2
        ).order_by("-materia"):
            votos_materia_od = []
            ...
            votacoes_od.append({"titulo": t_materia, "votos": votos_materia_od})
            ...
            context.update(
                {
                    ...
                    "decimo_quinto_ordenacao": dict_ord_template[
                        ordenacao.decimo_quinto
                    ],
...
class OcorrenciaSessaoView(FormMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        context = FormMixin.get_context_data(self, **kwargs)
        context["title"] = "Ocorrências da Sessão <small>(%s)</small>" % (self.object)
        sessao = context["object"]
        tipo_sessao = sessao.tipo
        if tipo_sessao.nome == "Solene":
            context.update({"subnav_template_name": "sessao/subnav-solene.yaml"})
    ...
    def delete(self):
        ...
        self.logger.info(
            "user="
            + username
            + ". OcorrenciaSessao com SessaoPlenaria de id={} deletada.".format(
                self.object.id
            )
        )
    ...
    def save(self, form):
        ...
        self.logger.info(
            "user="
            + username
            + ". OcorrenciaSessao de sessao_plenaria_id={} atualizada com sucesso.".format(
                self.object.id
            )
        )
...
class ConsideracoesFinaisView(FormMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        context = FormMixin.get_context_data(self, **kwargs)
        context["title"] = "Considerações Finais <small>(%s)</small>" % (self.object)
        sessao = context["object"]
        tipo_sessao = sessao.tipo
        if tipo_sessao.nome == "Solene":
            context.update({"subnav_template_name": "sessao/subnav-solene.yaml"})
    ...
    def delete(self):
        ...
        self.logger.info(
            "user="
            + username
            + ". ConsideracoesFinais com SessaoPlenaria de id={} deletada.".format(
                self.object.id
            )
        )
    ...
    def save(self, form):
        ...
        self.logger.info(
            "user="
            + username
            + ". consideracoesFinais de sessao_plenaria_id={} atualizada com sucesso.".format(
                self.object.id
            )
        )
...
class VotacaoEditView(SessaoPermissionMixin):
    ...
    def get(self, request, *args, **kwargs):
        ...
        votacao_existente = {
            "observacao": sub("&nbsp;", " ", strip_tags(votacao.observacao)),
    ...
    def get_success_url(self):
        ...
        pk = self.kwargs["pk"]
        return reverse("sapl.sessao:ordemdia_list", kwargs={"pk": pk}) + page
...
class VotacaoNominalAbstract(SessaoPermissionMixin):
    ...
    def get(self, request, *args, **kwargs):
        ...
        materia = {
            ...
            "ementa": sub("&nbsp;", " ", strip_tags(materia_votacao.materia.ementa)),
        }
        context = {
            "materia": materia,
        ...
        for parlamentar in Parlamentar.objects.filter(ativo=True):
            if parlamentar in presentes:
                try:
                    ...
                    username = self.request.user.username
                    self.logger.warning(
                        "User={}. Objeto voto_parlamentar do parlamentar de id={} não existe.".format(
                            username, parlamentar.pk
                        )
                    )
...
class VotacaoNominalEditAbstract(SessaoPermissionMixin):
    ...
    def get(self, request, *args, **kwargs):
        ...
        materia = {"materia": materia, "ementa": sub("&nbsp;", " ", strip_tags(ementa))}
        context.update({"materia": materia})
...
class VotacaoNominalExpedienteDetailView(DetailView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        materia = {
            "materia": expediente.materia,
            "ementa": sub("&nbsp;", " ", strip_tags(expediente.materia.ementa)),
        }
        context.update({"materia": materia})
    ...
    def get_success_url(self):
        ...
        pk = self.kwargs["pk"]
        return reverse("sapl.sessao:expedientemateria_list", kwargs={"pk": pk}) + page
...
class PautaSessaoView(TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        sessao = (
            SessaoPlenaria.objects.filter(publicar_pauta=True)
            .order_by("-data_inicio")
            .first()
        )
...
class PautaSessaoDetailView(PautaMultiFormatOutputMixin, DetailView):
    ...
    def get(self, request, *args, **kwargs):
        from sapl.relatorios.views import \
            relatorio_pauta_sessao_weasy  # Evitar import ciclico
        ...
        encerramento = (
            self.object.data_fim.strftime("%d/%m/%Y") if self.object.data_fim else ""
        )
        ...
        materias_expediente = []
        for m in (
            ExpedienteMateria.objects.prefetch_related("registrovotacao_set")
            ...
            if timezone.is_naive(data_hora_sessao):
                data_hora_sessao = timezone.make_aware(
                    data_hora_sessao, timezone.get_current_timezone()
                )
            data_hora_sessao_utc = data_hora_sessao.astimezone(timezone.utc)
            ultima_tramitacao = (
                m.materia.tramitacao_set.filter(timestamp__lt=data_hora_sessao_utc)
                .order_by("-data_tramitacao", "-id")
                .first()
                if m.tramitacao is None
                else m.tramitacao
            )
            ...
            materias_expediente.append(
                {
                    ...
                    "resultado": resultado,
                    "resultado_observacao": resultado_observacao,
                    "situacao": ultima_tramitacao.status
                    if ultima_tramitacao
                    else _("Não informada"),
                    "processo": f"{str(numeracao.numero_materia)}/{str(numeracao.ano_materia)}"
                    if numeracao
                    else "-",
                    "autor": [
                        str(x.autor)
                        for x in m.materia.autoria_set.select_related("autor").all()
                    ],
                    "turno": get_turno(ultima_tramitacao.turno)
                    if ultima_tramitacao
                    else "",
                }
            )
        ...
        expedientes = []
        for e in (
            ExpedienteSessao.objects.select_related("tipo")
            .filter(sessao_plenaria_id=self.object.id)
        ...
        materias_ordem = []
        for o in (
            OrdemDia.objects.prefetch_related("registrovotacao_set")
            .select_related("materia", "materia__tipo")
            .filter(sessao_plenaria_id=self.object.id)
        ):
            # Verificar resultado
            ...
            data_hora_sessao_utc = data_hora_sessao.astimezone(timezone.utc)
            ultima_tramitacao = (
                o.materia.tramitacao_set.filter(timestamp__lt=data_hora_sessao_utc)
                .order_by("-data_tramitacao", "-id")
                .first()
                if o.tramitacao is None
                else o.tramitacao
            )
...
class PesquisarSessaoPlenariaView(MultiFormatOutputMixin, FilterView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if not data:
            return HttpResponseRedirect(
                reverse(self.viewname) + f"?data_inicio__year={timezone.now().year}"
            )
...
def verifica_materia_sessao_plenaria_ajax(request):
    ...
    return JsonResponse(
        {
            "is_materia_presente": is_materia_presente,
            "is_materia_presente_any_sessao": is_materia_presente_any_sessao,
        }
    )
...
class LeituraEmBloco(PermissionRequiredForAppCrudMixin, ListView):
    ...
    def post(self, request, *args, **kwargs):
        if "marcadas_4" in request.POST:
            ...
            if not verifica_presenca(
                request, presenca_model, self.kwargs["pk"], True
            ) or not verifica_sessao_iniciada(request, self.kwargs["pk"], True):
                return self.get(request, self.kwargs)
            ...
            messages.add_message(
                self.request,
                messages.ERROR,
                _("Nenhuma matéria selecionada para leitura em Bloco"),
            )
            return self.get(request, self.kwargs)
...
class CorrespondenciaCrud(MasterDetailCrud):
    ...
    class BaseMixin(MasterDetailCrud.BaseMixin):
        list_field_names = [
            "numero_ordem",
            ("documento__data", "documento__interessado"),
            "documento",
            "documento__assunto",
        ]
            ...
            context.update(
                {
                    ...
                    if s.tipo.nome == "Solene"
                    else "sessao/subnav.yaml"
                }
            )
    ...
    class ListView(MasterDetailCrud.ListView):
        ...
        def hook_header_numero_ordem(self, *args, **kwargs):
            return (
                force_str(_("Ordem / Tipo"))
                if not self.request.user.is_anonymous
                else force_str(_("Tipo"))
            )
    ...
    class DetailView(MasterDetailCrud.DetailView):
        ...
        def hook_documento(self, obj, verbose_name=None, field_display=None):
            d = obj.documento
            url = reverse(
                "sapl.protocoloadm:documentoadministrativo_detail", kwargs={"pk": d.id}
            )
            return (verbose_name, f'<a href="{url}">{d}</a><br>{d.assunto}')
...
class CorrespondenciaEmLoteView(PermissionRequiredMixin, FilterView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["root_pk"] = self.kwargs["pk"]
        context["title"] = _("Correspondencias em Lote <small>({})</small>").format(s)
        ...
        if not len(qr):
            ...
            sessao_plenaria = SessaoPlenaria.objects.get(pk=self.kwargs["pk"])
            not_list = [self.kwargs["pk"]] + [
                m for m in sessao_plenaria.correspondencias.values_list("id", flat=True)
            ]
    ...
    def post(self, request, *args, **kwargs):
        ...
        max_numero_ordem = (
            Correspondencia.objects.filter(sessao_plenaria=self.kwargs["pk"]).aggregate(
                Max("numero_ordem")
            )["numero_ordem__max"]
            or 0
        )
        ...
        success_url = reverse(
            "sapl.sessao:correspondencia_list", kwargs={"pk": kwargs["pk"]}
        )
        return HttpResponseRedirect(success_url)
