...
from sapl.utils import (SEPARADOR_HASH_PROPOSICAO, MultiFormatOutputMixin,
                        autor_label, autor_modal, gerar_hash_arquivo,
                        get_base_url, get_client_ip,
                        get_mime_type_from_file_extension, get_tempfile_dir,
                        google_recaptcha_configured, lista_anexados,
                        mail_service_configured, montar_row_autor,
                        show_results_filter_set)

from .forms import (AcessorioEmLoteFilterSet, AcompanhamentoMateriaForm,
                    AdicionarVariasAutoriasFilterSet, AnexadaEmLoteFilterSet,
                    DespachoInicialForm, DocumentoAcessorioForm,
                    EtiquetaPesquisaForm, ExcluirTramitacaoEmLote,
                    ...
                    StatusTramitacaoFilterSet, TramitacaoEmLoteFilterSet,
                    TramitacaoEmLoteForm, UnidadeTramitacaoForm,
                    compara_tramitacoes_mat, filtra_tramitacao_destino,
                    filtra_tramitacao_destino_and_status,
                    filtra_tramitacao_status)
from .models import (AcompanhamentoMateria, Anexada, AssuntoMateria, Autoria,
                     ConfigEtiquetaMateriaLegislativa, DespachoInicial,
                     DocumentoAcessorio, HistoricoProposicao, MateriaAssunto,
...
def proposicao_texto(request, pk):
    ...
    if proposicao.texto_original:
        if (
            ...
            and not proposicao.autor.operadores.filter(id=request.user.id).exists()
        ):
            logger.error(
                "user="
                + username
    ...
    def get_context_data(self, **kwargs):
        ...
        try:
            ...
            materias_ano = MateriaLegislativa.objects.filter(
                ano=protocolo.ano, tipo=protocolo.tipo_materia
            ).latest("numero")
...
def recuperar_materia(request):
    ...
    try:
        ...
        numeracao = (
            sapl.base.models.AppConfig.objects.last().sequencia_numeracao_protocolo
        )
    ...
    if numeracao == "A":
        numero = MateriaLegislativa.objects.filter(ano=ano, tipo=tipo).aggregate(
            Max("numero")
        )
    elif numeracao == "L":
...
class PesquisarStatusTramitacaoView(FilterView):
    ...
    def get_filterset_kwargs(self, filterset_class):
        super(PesquisarStatusTramitacaoView, self).get_filterset_kwargs(filterset_class)
    ...
    def get_context_data(self, **kwargs):
        context = super(PesquisarStatusTramitacaoView, self).get_context_data(**kwargs)
    ...
    def get(self, request, *args, **kwargs):
        ...
        if (
            "descricao" in self.request.META["QUERY_STRING"]
...
class ProposicaoCrud(Crud):
    ...
    class DetailView(Crud.DetailView):
        ...
        def get_context_data(self, **kwargs):
            ...
            context["proposicao"] = Proposicao.objects.get(pk=self.kwargs["pk"])
            return context
            ...
            if p and p.autor.operadores.filter(id=request.user.id).exists():
                if action == "send":
                    if p.data_envio and p.data_recebimento:
                        ...
                        try:
                            self.logger.debug(
                                "User={}. Tentando obter número do objeto MateriaLegislativa "
                                "com atributos tipo={} e ano={}.".format(
                                    username, p.tipo.tipo_conteudo_related, p.ano
                                )
                            )
                            ...
                            if p.numero_materia_futuro:
                                ...
                                numero = (
                                    MateriaLegislativa.objects.filter(
                                        tipo=p.tipo.tipo_conteudo_related, ano=p.ano
                                    )
                                    .order_by("numero")
                                    .last()
                                    .numero
                            ...
                            messages.success(
                                request,
                                _(
                                    "{}: nº {} de {} <br>Atenção! Este número é apenas um provável "
                                    "número que pode não corresponder com a realidade".format(
                                        p.tipo, numero, p.ano
                                    )
                                ),
                            )
                    ...
                    if not p.data_envio:
                        self.logger.warning(
                            "User={}. Proposição (numero={}) ainda não foi enviada.".format(
                                username, p.numero_proposicao
                            )
                        )
                        msg_error = _("Proposição ainda não foi enviada.")
                    elif p.data_recebimento:
                        self.logger.warning(
                            "User={}. Proposição (numero={}) já foi recebida, "
                            "não é possível retorná-la.".format(
                                username, p.numero_proposicao
                            )
                        )
                        msg_error = _(
                        ...
                        if p.texto_articulado.exists():
                            ...
                            ta.save()
                        self.logger.info(
                            "User={}. Proposição (numero={}) Retornada com sucesso.".format(
                                username, p.numero_proposicao
                            )
                        )
                        messages.success(
        ...
        def dispatch(self, request, *args, **kwargs):
            ...
            try:
                self.logger.debug(
                    "User={}. Tentando obter objeto Proposicao com pk={}".format(
                        username, kwargs["pk"]
                    )
                )
                p = Proposicao.objects.get(id=kwargs["pk"])
                ...
                self.logger.warning(
                    "User={}. Erro ao obter proposicao com pk={}. Retornando 404. {}".format(
                        username, kwargs["pk"], str(e)
                    )
                )
                raise Http404()
        ...
        def _action_is_valid(self, request, *args, **kwargs):
            ...
            if proposicao:
                ...
                if proposicao[0][0] and proposicao[0][1]:
                    self.logger.warning(
                        "User={}. Proposição (id={}) já foi enviada e recebida."
                    ...
                    self.logger.warning(
                        ...
                        Use a opção Recuperar Proposição para depois excluí-la.""".format(
                            username, kwargs["pk"]
                        )
                    )
                    msg = _(
                        "Proposição já foi enviada mas ainda não recebida pelo protocolo. "
            ...
            if proposicao:
                ...
                if proposicao[0][0] and proposicao[0][1]:
                    ...
                    self.logger.warning(
                        """\
                        User={}. Proposição (id={}) já foi enviada, mas ainda não recebida pelo protocolo. \
                        Use a opção Recuperar Proposição para voltar para edição.""".format(
                            username, kwargs["pk"]
                        )
                    )
                    msg = _(
        ...
        def get_success_url(self):
            ...
            if tipo_texto == "T":
                ...
                self.logger.debug(
                    ...
                    você será redirecionado para a edição do Texto Eletrônico.""".format(
                        username
                    )
                )
                return reverse(
        ...
        def get_success_url(self):
            ...
            if tipo_texto == "T":
                ...
                self.logger.debug(
                    ...
                    User={}. Sempre que uma Proposição é inclusa ou alterada e a opção "Texto Articulado" for marcada, \
                    você será redirecionado para o Texto Eletrônico. \
                    Use a opção "Editar Texto" para construir seu texto.""".format(
                        username
                    )
                )
                return reverse(
...
class HistoricoProposicaoView(PermissionRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        if not user.is_superuser and grupo_autor.user_set.filter(id=user.id).exists():
...
class RelatoriaCrud(MasterDetailCrud):
    ...
    class CreateView(MasterDetailCrud.CreateView):
        ...
        def get_initial(self):
            ...
            loc_atual = (
                Tramitacao.objects.filter(materia=materia)
            ...
            if loc_atual is None:
                ...
                if comissao:
                    ...
                    return {
                        "comissao": localizacao,
                        "tipo_unidade_tramitacao_destino": tipo_unidade_tramitacao_destino,
...
class TramitacaoCrud(MasterDetailCrud):
    ...
    class DeleteView(MasterDetailCrud.DeleteView):
        ...
        def delete(self, request, *args, **kwargs):
            tramitacao = Tramitacao.objects.select_related("materia").get(
                id=self.kwargs["pk"]
            )
            ...
            if tramitacao.pk != ultima_tramitacao.pk:
                ...
                penultima_tramitacao = (
                    materia.tramitacao_set.order_by("-data_tramitacao", "-id")
                    .exclude(id=ultima_tramitacao.id)
                    .first()
                )
                if (
                    not penultima_tramitacao
                    or penultima_tramitacao.status.indicador == "F"
                ):
                ...
                if tramitar_anexadas:
                    ...
                    for materia in materias_anexadas:
                        ...
                        if compara_tramitacoes_mat(ultima_tramitacao, tramitacao):
                            tramitacoes_deletar.append(ultima_tramitacao.id)
                            # recupera últimas duas tramitacoes
                            penultima_tramitacao = (
                                materia.tramitacao_set.order_by(
                                    "-data_tramitacao", "-id"
                                )
                                .exclude(id=ultima_tramitacao.id)
                                .first()
...
class DocumentoAcessorioCrud(MasterDetailCrud):
    ...
    class CreateView(MasterDetailCrud.CreateView):
        ...
        def get_context_data(self, **kwargs):
            context = super(MasterDetailCrud.CreateView, self).get_context_data(
                **kwargs
            )
            return context
...
class MateriaLegislativaPesquisaView(MultiFormatOutputMixin, FilterView):
    ...
    def hook_texto_original(self, obj):
        ...
        texto_original = (
            obj.texto_original if not isinstance(obj, dict) else obj["texto_original"]
        )
    ...
    def get_filterset_kwargs(self, filterset_class):
        ...
        if "o" in self.request.GET and not self.request.GET["o"]:
            args = (
                ["-ano", "tipo__sequencia_regimental", "-numero"]
                if BaseAppConfig.attr("ordenacao_pesquisa_materia") == "R"
                else ["-ano", "tipo__sigla", "-numero"]
            )
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.primeira_tramitacao:
            ...
            context["object_list"] = [
                ...
                if obj.tramitacao_set.order_by("-data_tramitacao", "-id").all().count()
                == 0
            ]
        else:
            ...
            context["form"].fields[
                "unidade_tramitacao_local"
            ].initial = UnidadeTramitacao.objects.get(
...
class EtiquetaPesquisaView(PermissionRequiredMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        if form.cleaned_data["processo_inicial"]:
            materias = materias.filter(
                numero__gte=form.cleaned_data["processo_inicial"],
                numero__lte=form.cleaned_data["processo_final"],
            )
...
class ExcluirTramitacaoEmLoteView(PermissionRequiredMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        for tramitacao in tramitacao_set:
            ...
            if (
                tramitacao
                == materia.tramitacao_set.order_by("-data_tramitacao", "-id").first()
            ):
                tramitacao.delete()
...
def create_zip_docacessorios(materia):
    ...
    """
    logger = logging.getLogger(__name__)
    docs = materia.documentoacessorio_set.all().values_list("arquivo", flat=True)
    if not docs:
        return None, None
    ...
    logger.info(
        "Gerando compilado PDF de documentos acessorios com {} documentos".format(
            docs_path
        )
    )
...
def get_zip_docacessorios(request, pk):
    ...
    try:
        ...
        messages.add_message(request, messages.ERROR, msg)
        return redirect(
            reverse("sapl.materia:documentoacessorio_list", kwargs={"pk": pk})
        )
    except Exception as e:
        logger.error(
            "user={}. Um erro inesperado ocorreu na criação do pdf de documentos acessorios: {}".format(
                username, str(e)
            )
        )
    ...
    response["Content-Disposition"] = 'attachment; filename="%s"' % external_name
    return response
...
def create_pdf_docacessorios(materia):
    ...
    merged_pdf = "{}/mat_{}_{}_docacessorios.pdf".format(
        get_tempfile_dir(), materia.pk, time.mktime(datetime.now().timetuple())
    )
