...
from sapl.materia.forms import (TramitacaoForm, TramitacaoUpdateForm,
...
def make_norma():
    ...
    tipo = baker.make(TipoNormaJuridica, sigla="T1", descricao="Teste_Tipo_Norma")
    baker.make(
        NormaJuridica,
        tipo=tipo,
        numero=1,
...
def test_materia_anexada_submit(admin_client):
    ...
    tipo_anexada = baker.make(TipoMateriaLegislativa, sigla="T2", descricao="Teste_2")
    regime_tramitacao = baker.make(RegimeTramitacao, descricao="Teste_Regime")
    baker.make(
        MateriaLegislativa,
        tipo=tipo_anexada,
...
def test_documento_acessorio_submit(admin_client):
    ...
    tipo = baker.make(TipoDocumento, descricao="Teste")

    arquivo = SimpleUploadedFile(
        "file.pdf", b"conteudo do arquivo", content_type="application/pdf"
    )
...
def test_numeracao_materia_legislativa_por_legislatura(admin_client):
    ...
    tipo_materia = baker.make(TipoMateriaLegislativa, id=1, sequencia_numeracao="L")
    materia = baker.make(
        MateriaLegislativa,
        tipo=tipo_materia,
        ano=2017,
...
def test_numeracao_materia_legislativa_por_ano(admin_client):
    # Cria uma materia
    tipo_materia = baker.make(TipoMateriaLegislativa, id=1, sequencia_numeracao="A")
    materia = baker.make(MateriaLegislativa, tipo=tipo_materia, ano=2017, numero=1)
...
def test_tramitacoes_materias_anexadas(admin_client):
    ...
    status = baker.make(StatusTramitacao, indicador="R")
    ...
    assert (
        materia_principal.tramitacao_set.order_by("-data_tramitacao", "-id").first()
        == tramitacao_principal
    )
    ...
    assert (
        materia_anexada.tramitacao_set.order_by("-data_tramitacao", "-id").first()
        == tramitacao_anexada
    )
    assert (
        materia_principal.tramitacao_set.order_by("-data_tramitacao", "-id")
        .all()
        .count()
        == 1
    )
