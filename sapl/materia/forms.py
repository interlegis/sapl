...
from sapl.materia.models import (AssuntoMateria,
                                 ...
                                 HistoricoProposicao, MateriaAssunto,
...
from sapl.utils import (SEPARADOR_HASH_PROPOSICAO, YES_NO_CHOICES,
...
class MateriaLegislativaForm(FileFieldCheckMixin, ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance and self.instance.pk:
            ...
            if (
                kwargs["instance"].numero_protocolo
                and Protocolo.objects.filter(
                    numero=kwargs["instance"].numero_protocolo,
                    ano=kwargs["instance"].ano,
                ).exists()
            ):
        ...
        if protocolo:
            ...
            if protocolo_antigo != protocolo:
                exist_materia = MateriaLegislativa.objects.filter(
                    numero_protocolo=protocolo, ano=ano
                ).exists()

                exist_doc = DocumentoAdministrativo.objects.filter(
                    protocolo_id=protocolo, ano=ano
                ).exists()
                ...
                if p.tipo_materia != cleaned_data["tipo"]:
                    self.logger.error(
                        "Tipo do Protocolo ({}) deve ser o mesmo do Tipo Matéria ({}).".format(
                            cleaned_data["tipo"], p.tipo_materia
                        )
                    )
                    raise ValidationError(
...
class DocumentoAcessorioForm(FileFieldCheckMixin, ModelForm):
    ...
    class Meta:
        ...
        fields = ["tipo", "nome", "data", "autor", "ementa", "indexacao", "arquivo"]
    ...
    def clean(self):
        ...
        if not self.instance.data_tramitacao:
            ...
            if cleaned_data["data_tramitacao"] > timezone.now().date():
                self.logger.warning(
                    "A data de tramitação informada ({}) não é menor ou igual a data de hoje!".format(
        ...
        if data_enc_form:
            if data_enc_form < data_tram_form:
                ...
                self.logger.warning(
                    "A data de encaminhamento ({}) deve ser maior que a data de tramitação! ({})".format(
    ...
    def save(self, commit=True):
        ...
        if tramitar_anexadas:
            ...
            for mat in materias_anexadas:
                ultima_tramitacao = (
                    mat.tramitacao_set.select_related("unidade_tramitacao_destino")
                    .order_by("-data_tramitacao", "-id")
                    .first()
                )
                if (
                    not ultima_tramitacao
                    or ultima_tramitacao.unidade_tramitacao_destino
...
class TramitacaoUpdateForm(TramitacaoForm):
    ...
    def clean(self):
        ...
        if ultima_tramitacao != obj:
            if cd[
                "unidade_tramitacao_destino"
            ] != obj.unidade_tramitacao_destino and BaseAppConfig.attr(
        ...
        if not (
            ...
            or cd["urgente"] != str(obj.urgente)
            or cd["turno"] != obj.turno
        ):
...
class MateriaLegislativaFilterSet(django_filters.FilterSet):
    ...
    def __init__(self, *args, **kwargs):
        ...
        row6 = to_row([("relatoria__parlamentar_id", 6), ("em_tramitacao", 6)])
        row7 = to_row(
            [
        ...
        row8 = to_row([("o", 8), ("tipo_listagem", 4)])
        ...
        row11 = to_row([("data_origem_externa", 8), ("local_origem_externa", 4)])
...
class TipoProposicaoForm(ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            "tipo_conteudo_related": forms.HiddenInput(),
    ...
    def clean(self):
        ...
        if (
            ...
            .objects.filter(pk=cd["tipo_conteudo_related"])
            .exists()
        ):
            self.logger.error(
    ...
    def save(self, commit=False):
        ...
        tipo_proposicao.tipo_conteudo_related = (
            tipo_proposicao.content_type.model_class().objects.get(
                pk=self.cleaned_data["tipo_conteudo_related"]
            )
        )
...
class TramitacaoEmLoteForm(ModelForm):
    ...
    def save(self, commit=True):
        ...
        ultima_edicao = (
            self.initial["ultima_edicao"] if "ultima_edicao" in self.initial else ""
        )
        ...
        for mat_id in materias:
            ...
            if tramitar_anexadas:
                ...
                for ml in anexadas:
                    if (
                        not ml.tramitacao_set.order_by("-data_tramitacao", "-id").all()
                        or ml.tramitacao_set.order_by("-data_tramitacao", "-id")
                        .first()
                        .unidade_tramitacao_destino
                        == tramitacao.unidade_tramitacao_local
                    ):
...
class ProposicaoForm(FileFieldCheckMixin, forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.receber_recibo = BaseAppConfig.attr("receber_recibo_proposicao")
        ...
        fields = [
            to_column((Fieldset(TipoProposicao._meta.verbose_name, Field("tipo")), 12)),
            to_column(
                (
                    Alert(
                        "teste",
        ...
        fields.append(
            to_column(
                (
                    Fieldset(
                        _(
                            "Outras informações - Vincular a Matéria Legislativa Existente"
                        ),
                        to_row(
                            [
                                ("tipo_materia", 12),
                        ...
                        to_row([("numero_materia", 6), ("ano_materia", 6)]),
                    ),
                    12,
                )
            ),
    ...
    def save(self, commit=True):
        ...
        sequencia_numeracao = BaseAppConfig.attr("sequencia_numeracao_proposicao")
...
class DevolverProposicaoForm(forms.ModelForm):
    ...
    class Meta:
        model = Proposicao
        fields = ["justificativa_devolucao", "observacao", "user", "ip"]
        widgets = {
            "user": forms.HiddenInput(),
            "ip": forms.HiddenInput(),
        }
...
class ConfirmarProposicaoForm(ProposicaoForm):
    ...
    class Meta:
        ...
        widgets = {
            "descricao": widgets.Textarea(attrs={"readonly": "readonly", "rows": 4}),
    ...
    def __init__(self, *args, **kwargs):
        self.proposicao_incorporacao_obrigatoria = BaseAppConfig.attr(
        ...
        if (
            not BaseAppConfig.objects.last().escolher_numero_materia_proposicao
            or not self.instance.numero_materia_futuro
        ):
            if "numero_materia_futuro" in self._meta.fields:
                del fields[0][0][3]
                ...
                to_row(
                    [
                        ...
                        (
                            Alert(
                                _(
                                    "O responsável pela incorporação pode "
        ...
        if self.instance.numero_materia_futuro:
            self.fields[
                "numero_materia_futuro"
            ].initial = self.instance.numero_materia_futuro
    ...
    def save(self, commit=False):
        ...
        if self.instance.tipo.content_type.model_class() == TipoMateriaLegislativa:
            ...
            if numeracao == "A":
                ...
                numero = MateriaLegislativa.objects.filter(tipo=tipo).aggregate(
                    Max("numero")
                )
            ...
            if cd["numero_materia_futuro"] and not MateriaLegislativa.objects.filter(
                tipo=tipo, ano=ano, numero=cd["numero_materia_futuro"]
            ):
            ...
            self.instance.results["messages"]["success"].append(
                _("Autoria registrada para (%s)") % str(autoria.autor)
            )
        ...
        if numeracao == "A":
            nm = Protocolo.objects.filter(ano=timezone.now().year).aggregate(
                Max("numero")
            )
        elif numeracao == "L":
            legislatura = Legislatura.objects.filter(
            ...
            nm = Protocolo.objects.filter(
                ano__gte=ano_inicio, ano__lte=ano_fim
            ).aggregate(Max("numero"))
        else:
...
class StatusTramitacaoFilterSet(django_filters.FilterSet):
    ...
    def multifield_filter(self, queryset, name, value):
        return queryset.filter(
            Q(sigla__icontains=value) | Q(descricao__icontains=value)
        )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.form.helper.layout = Layout(
            Fieldset(
                _("Pesquisa de Status de Tramitacao"),
                row0,
                form_actions(label="Pesquisar"),
            )
        )
