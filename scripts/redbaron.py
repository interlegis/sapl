...
def get(r):
    return [s for s in r("string") if not inter(s) and s.value not in mark.ok]
    # and frase(s)
...
def local(node):
    res = "%s:%s" % (
        node.git_project_root.__filename__,
...
def acha_props_constantes(red):
    ...
    for fun in red("def"):
        if fun.decorators.dumps().strip() == "@property":
            ...
            if len(nos) == 1:
                [ret] = nos
                if isinstance(ret, ReturnNode) and isinstance(ret.value, StringNode):
                    yield fun
...
def corrige_props_constantes(reds):
    ...
    for p in pp:
        p.parent.value[p.index_on_parent] = "{} = {}".format(
            p.name, p("return")("string").dumps()
        )
        write(p)
